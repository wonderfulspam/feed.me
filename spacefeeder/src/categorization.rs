...
impl CategorizationEngine {
    ...
    pub fn generate_tags_for_item(
        ...
        let feed_tag_hints: HashSet<String> = if let Some(feed_tags) = feed_tags {
        ...
        for rule in &self.config.rules {
            if rule.rule_type == "exclude_if" {
                ...
                if rule.patterns.iter().any(|p| self.matches_keyword(&content, &p.to_lowercase())) {
        ...
        for feed_tag in &feed_tag_hints {
            if !seen.contains(feed_tag) && !excluded_tags.contains(feed_tag) {
                ...
                if let Some(tag_def) = self.config.tags.iter().find(|t| self.normalize_tag(&t.name) == *feed_tag) {
                    if !tag_def.keywords.is_empty() {
                        let has_any_keyword = tag_def.keywords.iter()
    ...
    fn apply_rule(
        ...
        let matches = match rule.rule_type.as_str() {
            "title_contains" => {
                ...
                rule.patterns.iter().any(|p| self.matches_keyword(&title_lower, &p.to_lowercase()))
            }
            "content_contains" => {
                let content = format!(
                    "{} {}",
                ...
                rule.patterns.iter().any(|p| self.matches_keyword(&content, &p.to_lowercase()))
            }
            "content_analysis" => {
...
mod tests {
    use super::*;
    use crate::config::{TagDefinition, TagAlias, TagRule, CategorizationConfig};
        ...
        let config = CategorizationConfig {
            ...
            tags: vec![
                TagDefinition {
                    ...
                    keywords: vec!["artificial intelligence".to_string(), "machine learning".to_string(), "neural network".to_string()],
                },
                TagDefinition {
                    name: "rust".to_string(),
                ...
                TagDefinition {
                    ...
                    keywords: vec!["python".to_string(), "django".to_string(), "pip".to_string()],
                },
            ],
            rules: vec![
                ...
                TagRule {
                    ...
                    required_keywords: vec!["artificial intelligence".to_string(), "machine learning".to_string()],
                    exclude_tags: vec![],
                },
    ...
    fn test_keyword_word_boundaries() {
        ...
        assert!(engine.matches_keyword("artificial intelligence is growing", "artificial intelligence"));
    ...
    fn test_author_with_content_requires_keywords() {
        ...
        assert!(ai_tags.is_empty(), "Should not tag non-AI content from Simon Willison with AI");
        ...
        let ai_tags: Vec<_> = tags.iter().filter(|t| t.name == "ai").collect();
        assert!(!ai_tags.is_empty(), "Should tag AI content from Simon Willison with AI");
        assert!(ai_tags[0].confidence > 0.5, "Should have reasonable confidence for AI content");
    }
    ...
    fn test_title_ai_matching_precision() {
        ...
        let tags = engine.generate_tags_for_item(
            ...
            Some("Hacker News"),
            "hackernews",
            Some(&["news".to_string(), "tech".to_string()]),
            None,
        );
        ...
        let ai_tags: Vec<_> = tags.iter().filter(|t| t.name == "ai").collect();
        assert!(ai_tags.is_empty(), "Should not tag articles just because they contain 'said'");
    }
    ...
    fn test_content_analysis_multi_keyword_requirement() {
        ...
        if !rust_tags.is_empty() {
            ...
            assert!(rust_tags[0].confidence < 0.7, "Should have low confidence from keyword match, not high confidence from rule");
        }
        ...
        assert!(rust_tags[0].confidence >= 0.7, "Should have high confidence from content_analysis rule");
    }
    ...
    fn test_exclude_if_prevents_tagging() {
        ...
        assert!(python_tags.is_empty(), "Should not tag Python in weekly links");
    ...
    fn test_feed_level_tags_should_not_override_content() {
        ...
        assert!(tag_names.contains(&"python".to_string()), "Should tag Python content");
    ...
    fn test_realistic_hacker_news_false_positives() {
        ...
        let ai_tags: Vec<_> = tags.iter().filter(|t| t.name == "ai").collect();
        assert!(ai_tags.is_empty(), "Renewable energy articles should not get AI tag");
        ...
        let ai_tags: Vec<_> = tags.iter().filter(|t| t.name == "ai").collect();
        assert!(ai_tags.is_empty(), "Corporate handbooks should not get AI tag");
    ...
    fn test_tag_normalization() {
        ...
        assert_eq!(normalized, "ai", "artificial-intelligence should normalize to AI");
