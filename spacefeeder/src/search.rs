...
impl SearchIndex {
    ...
    pub fn search_with_filters(
        ...
        let filtered_results: Vec<SearchResult> = results
            .into_iter()
            .filter(|result| {
                let author_matches = author_filter
                    .map_or(true, |filter| result.author.to_lowercase().contains(&filter.to_lowercase()));
                let tier_matches = tier_filter
                    .map_or(true, |filter| result.tier == filter);
...
mod tests {
    ...
    fn test_search_with_filters() {
        ...
        // Test tier filter
        let results = search_index.search_with_filters("rust", None, Some("love"), 10).unwrap();
        assert!(results.iter().all(|r| r.tier == "love"));
        
        // Test author filter
        let results = search_index.search_with_filters("rust", Some("Rust"), None, 10).unwrap();
